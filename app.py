...
CSS_PATH = "style.css"
MODEL_PATH = "mon_modele.keras" # Chemin vers votre modèle Keras
...
def load_keras_model(model_path):
    """Charge le modèle Keras en cache pour éviter les rechargements."""
    try:
        if os.path.exists(model_path):
            model = tf.keras.models.load_model(model_path)
            return model
        else:
            st.error(f"Erreur Critique : Fichier modèle non trouvé à '{model_path}'. Le service d'analyse est inopérant.")
            return None
    except Exception as e:
        st.error(f"Erreur lors du chargement du modèle Keras : {e}. Assurez-vous que le fichier est valide.")
        return None
...
def show_chatbot_view(groq_client):
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"], avatar='🧑‍🌾' if message["role"] == 'user' else '🤖'):
            ...
            # The audio display part for user messages that had audio is removed since input method changed
            # if 'audio_data' in message and message["role"] == 'user':
            #     try:
            #         decoded_audio = base64.b64decode(message['audio_data'])
            #         st.audio(decoded_audio, format=message.get('audio_mime_type', 'audio/webm'), start_time=0)
            #     except Exception as e:
            #         st.warning(f"Impossible de lire l'audio de l'historique : {e}")

    st.markdown("<div style='margin-top: 1.5rem;'></div>", unsafe_allow_html=True)

    # --- Removed: Custom Audio Recorder Component and its label ---
    # st.markdown("<h4>Ou enregistrez votre voix :</h4>", unsafe_allow_html=True)
    # recorded_audio_component_value = audio_recorder_component()
    ...
    # audio_to_display_base64 = None # Not needed without audio recording
    # audio_mime_type_to_display = None # Not needed without audio recording

    # --- Removed: Logic for processing recorded audio from the custom component ---
    # if recorded_audio_component_value and isinstance(recorded_audio_component_value, dict) and \
    #    'audio_data' in recorded_audio_component_value and 'mime_type' in recorded_audio_component_value:
